...
udp_socket_t open_udp_socket()
{
        ...
        if (s == -1) {
                rcom_err("open_udp_socket: failed to create the socket");
        ...
        if (ret == -1) {
                rcom_err("open_udp_socket: failed to bind the socket");
                close(s);
                return INVALID_UDP_SOCKET;
        }
...
int udp_socket_send(udp_socket_t socket, addr_t *addr, data_t *data)
{
        ...
        if (ret == -1) {
                char b[64];
                rcom_err("messenger_send: sendto() to %s failed: %s",
...
int udp_socket_read(udp_socket_t socket, data_t *data, addr_t *addr)
{
        ...
        len = recvfrom(socket,
                       ...
                       &addrlen);
        if (len < 0) {
                rcom_err("udp_socket_read: recvfrom failed");
                return -1;
        }
...
tcp_socket_t open_tcp_socket(addr_t *addr)
{
        ...
        if (s == -1) {
                rcom_err("open_tcp_socket: failed to create the socket");
                return -1;
        }

        ret = connect(s, (struct sockaddr *) addr, addrlen);
        if (ret < 0) {
                rcom_err("open_tcp_socket: failed to bind the socket");
                return -1;
        }
...
int tcp_socket_send(tcp_socket_t socket, const char *data, int len)
{
        ...
        if (socket < 0) {
                rcom_err("http_send: invalid socket");
                return -1;
        }

        //rcom_debug("http_send: sending: %.*s", len, data);
        ...
        while (sent < len) {
                ...
                //rcom_err("socket_send: len=%d, sent=%d, len-send=%d, ret=%d",
                // len, sent, len - sent, ret);
                if (ret < 0) {
                        rcom_err("socket_send: send failed: %s", strerror(errno));
                        return -1;
                }
                if (ret == 0) {
                        rcom_err("socket_send: send() returned zero");
                        return -1;
                }
                sent += ret;
        }
...
int tcp_socket_recv(tcp_socket_t socket, char *data, int len)
{
        ...
        if (received < 0) {
                rcom_err("tcp_socket_recv: recv failed: %s", strerror(errno));
                return (errno == EAGAIN)? -2 : -1;
        } else if (received == 0) {
                rcom_err("tcp_socket_recv: socket shut down");
                return 0;
        }
        //rcom_debug("http_recv: len=%d data='%.*s'", received, received, data);        
        return received;
}
...
tcp_socket_t open_server_socket(addr_t* addr)
{
        ...
        if (s == -1) {
                rcom_err("open_server_socket: failed: %s", strerror(errno));
                return INVALID_TCP_SOCKET;
        }

        ret = bind(s, (struct sockaddr *) addr, socklen);
        if (ret == -1) {
                rcom_err("open_server_socket: bind failed: %s", strerror(errno));
                return INVALID_TCP_SOCKET;
        }
        ...
        ret = listen(s, 10);
        if (ret == -1) {
                rcom_err("open_server_socket: listen failed: %s", strerror(errno));
                return INVALID_TCP_SOCKET;
        }
...
tcp_socket_t server_socket_accept(tcp_socket_t s)
{
        ...
        if (posix_wait_data(s, 1)) {
                //rcom_debug("server_socket_accept: calling accept");
                client =  accept(s, (struct sockaddr*) &addr, &addrlen);
                if (client == -1) {
                        // Server socket is probably being closed
                        // FIXME: is this true?
                        rcom_err("server_socket_accept: accept failed: %s (socket %d)",
                                strerror(errno), s); 
                        return INVALID_TCP_SOCKET;
                }
                //rcom_debug("server_socket_accept: new connection");
                return client;
        } else
...
static int posix_wait_data(int socket, int timeout)
{
        ...
        if (timeout < 0) {
                rcom_err("posix_wait_data: invalid value for timeout: %d", timeout);
                return -1;
        }
        ...
        if (ret < 0) {
                rcom_err("udp_socket_wait_data: error: %s", strerror(errno));
                return -1;
        } else if (ret == 0) {
                //rcom_err("http_wait_data: time out");
                return 0;
        }
        return 1; 
}
