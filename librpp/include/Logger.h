...
namespace rpp
{
    ...
    public:
        ...
        void log(log_level level, const std::string_view& message) override;
        std::string get_log_file_path() override;
        void set_application_name(std::string_view application_name) override;

        void log_to_file(std::string_view log_path) override;
...
void r_err(const std::string format, Args && ...args) {
    std::string message;
    StringUtils::string_printf(message, format.c_str(), std::forward<Args>(args)...);
