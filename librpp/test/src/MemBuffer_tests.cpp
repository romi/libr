...
TEST_F(MemBuffer_tests, append_zero_appends_zero)
{
    // Arrange
    rpp::MemBuffer MemBuffer;
    const int bufflen = 5;
    ...
    // Act
    MemBuffer.append(buff, bufflen);
    MemBuffer.append_zero();
    auto actual = MemBuffer.data();

    //Assert
    ASSERT_EQ(actual.size(), bufflen+1);
    ASSERT_EQ(actual[0], 't');
    ASSERT_EQ(actual[1], 'e');
    ASSERT_EQ(actual[2], 's');
    ASSERT_EQ(actual[3], 't');
...
TEST_F(MemBuffer_tests, clear_clears_data_size_correct)
{
    ...
    // Act
    MemBuffer.printf("%s", expected.c_str());
    auto expected_string = MemBuffer.string();
    MemBuffer.clear();
    auto actual = MemBuffer.data();
...
TEST_F(MemBuffer_tests, mutex_is_valid)
{
    // Arrange
    auto actual = false;
    rpp::MemBuffer MemBuffer;
